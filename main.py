...
def main():
    ...
    if type(a) != type("a".encode('utf-8')):
        a = a.encode('utf-8')
    safeprint(verify(a))
    safeprint("Malformed bounty 2")
    test = Bounty('8.8.8:8888',"1JTGcHS3GMhBGGww4ZEDmP7u9",1090)
    safeprint(test)
    safeprint(test.isValid())
    a = pickle.dumps(test,1)
    if type(a) != type("a".encode('utf-8')):
